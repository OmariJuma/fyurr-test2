...
import babel
from flask import Flask, render_template, request, Response, flash, redirect, session, url_for,jsonify
...
class Venue(db.Model):
    ...
    seeking_talent=db.Column(db.String(10),  default=False)
...
def format_datetime(value, format='medium'):
   #I had trouble rendering the date because the date variable I have commented below wanted string values only  
...
def venues():
  ...
  list_venue = Venue.query.distinct(Venue.city, Venue.state).order_by(asc(Venue.city)).all()
  data = []
  # This loop is to iterate the list venue and filter
  for venue in list_venue:
    ...
    # Use a for loop to append in the filtered items to filtered list
    filtered = []
    for venue in venues:
      filtered.append({
        "id": venue.id,"name": venue.name})
      
    city_state["venues"] = filtered
...
def search_venues():
  ...
  Venue.name.ilike('%{}%'.format(search_item))).all()
...
def show_venue(venue_id):
  ...
  # selects shows that have a date and time value less than the current time to be past shows
  ...
  # iteriate through past shows then attach the required rows of each relation column  
  shows = []
  ...
  # set venue as the value of past_shows and total number of upcoming shows as past shows and past_shows_count
...
def create_venue_submission():
  if request.method == 'POST':
    ...
    venue=Venue(name=name,city=city,state=state,address=address,phone=phone,image_link=img, facebook_link=fb,website_link=web,genres=genres, seeking_talent=seeking,seeking_description=desc)
...
def edit_venue_submission(venue_id):
  ...
  if request.method == 'POST':
    ...
    db.session.rollback()
    print(sys.exc_info())
    flash("Ooops, Venue could not be edited.")
...
def show_artist(artist_id):
  ...
  # selects shows that have a date and time value greater than the current time to be upcoming shows
  ...
  # set artist as the value of past shows and total number of upcoming shows as past_shows and past_shows_count
  ...
  # set artist as the value of upcoming shows and upcoming shows upcoming_shows_count
...
def create_artist_submission():
  ...
  desc=request.form.get('seeking_description')
  if seeking!=None:
    seeking=True
  else:
    seeking=False
  ...
  artist=Artist(name=name,city=city,state=state,phone=phone,image_link=img,genres=genres,facebook_link=fb,website_link=web,seeking_venue=seeking,seeking_description=desc)
  db.session.add(artist)
  db.session.commit()
  # on successful db insert, flash success
...
def create_show_submission():
  ...
  db.session.add(show)
  db.session.commit()
  #session.query(artist_id, venue_id).join(Show).filter(Artist.artist_id == Venue.venue_is).all():
